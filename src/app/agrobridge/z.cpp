...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        if (emailChanged && !showEmailVerification) {
          success("Profile updated successfully! Email was not changed as it requires verification.")
          setUserData((prev) => ({ ...prev, email: originalEmail }))
          setEmailChanged(false)
        } else {
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <p className="text-sm text-yellow-800 mb-2">
                            Email verification required for this change (Save Changes won&apos;t update email until
                            verified)
                          </p>
