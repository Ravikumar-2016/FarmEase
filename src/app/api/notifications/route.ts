...
interface CollectionMethods {
  insertOne: (doc: Notification) => Promise<{ insertedId: ObjectId }>;
  find: (query: object) => {
    sort: (sort: object) => {
      limit: (limit: number) => {
        toArray: () => Promise<Notification[]>;
      };
    };
  };
...
interface Database {
  collection: (name: string) => CollectionMethods;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    let updateQuery: NotificationQuery = { userId };
    ...
    const result = await db
      .collection("notifications")
      .updateMany(updateQuery, { $set: { isRead: true } });
